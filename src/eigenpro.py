...
def asm_eigenpro_fn(samples, map_fn, top_q, bs_gpu, alpha, min_q=5, seed=1):
    ...
    scale = svd.float_x(np.power(eigvals[0] / tail_eigval, alpha))
    ...
    # change 2 - commented this
#     knorms = 1 - np.sum(eigvecs ** 2, axis=1) * n_sample
#     beta = np.max(knorms)

    return eigenpro_fn, scale, eigvals[0], svd.float_x(beta)
...
class FKR_EigenPro(nn.Module):
    ...
    def evaluate(self, x_eval, y_eval, bs,
        ...
        #print(p_eval.shape, y_eval.shape)
        #print("ERROR: " , np.mean(np.power(p_eval - y_eval, 2)))
...
#             print("test shape",y_eval.shape)
#             print("pred shape",p_eval.shape)
    ...
    def fit(self, x_train, y_train, x_val, y_val, epochs, mem_gb,
        ...
        if n_subsamples is None:
            ...
            #if n_samples < 1000000:
                n_subsamples = min(n_samples, 4000)
        ...
        for epoch in epochs:
            ...
            if run_epoch_eval:
                ...
                if classification:
                    print("train acc: %.5f\t val acc: %.5f (%d epochs, %.2f seconds)\t"\
                          %(tr_score['multiclass-acc']*100, tv_score['multiclass-acc']*100, epoch, train_sec))
                else:
                    print("train error: %.5f\t val error: %.5f (%d epochs, %.2f seconds)\t"\
                          %(np.sqrt(tr_score['mse']), np.sqrt(tv_score['mse']), epoch, train_sec))
